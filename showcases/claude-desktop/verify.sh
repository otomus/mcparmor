...
# Verifies MCP Armor protection is active for Claude Desktop tools.
#
# Checks:
#   1. mcparmor is installed and on PATH
#   2. The broker reports active status for the claude-desktop host
#   3. Secret scanning is active — a response containing a raw secret is redacted

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "=== MCP Armor Claude Desktop Verification ==="
echo ""
...
# Check 2: broker status for claude-desktop
echo ""
echo "=== Broker status ==="
...
# This sends a ping request through a sandboxed node process whose response
# contains a raw Anthropic-style API key. The broker should intercept the
# response and redact the key before it reaches the MCP host.
#
# We use the github.armor.json profile because it has scan_secrets: true,
# which is representative of community profiles that handle sensitive data.
...
if echo "${RESPONSE}" | grep -q "REDACTED"; then
  ...
  echo ""
  echo "If secret scanning is expected to be active, check that scan_secrets: true"
  echo "is set in the armor profile at ${ARMOR_PROFILE}"
  exit 1
fi
