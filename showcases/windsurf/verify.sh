...
# Verifies MCP Armor protection is active for Windsurf tools.
#
# Checks:
#   1. mcparmor is installed and on PATH
#   2. The broker reports active status for the windsurf host
#   3. Secret scanning is active — a response containing a raw secret is redacted

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "=== MCP Armor Windsurf Verification ==="
echo ""
...
# Check 2: broker status for windsurf
echo ""
echo "=== Broker status ==="
mcparmor status --host windsurf
