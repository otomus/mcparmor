...
# Verifies MCP Armor protection is active for Arqitect tools.
#
# Checks:
#   1. mcparmor is installed and on PATH
#   2. The broker reports active status for the arqitect host
#   3. Secret scanning is active — a response containing a raw secret is redacted

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "=== MCP Armor Arqitect Verification ==="
echo ""
...
# Check 2: broker status for arqitect
echo ""
echo "=== Broker status ==="
mcparmor status --host arqitect
