...
# Verifies MCP Armor protection is active for VS Code tools.
#
# Checks:
#   1. mcparmor is installed and on PATH
#   2. The broker reports active status for the vscode-project host
#   3. Secret scanning is active — a response containing a raw secret is redacted

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "=== MCP Armor VS Code Verification ==="
echo ""
...
# Check 2: broker status for vscode-project
echo ""
echo "=== Broker status ==="
