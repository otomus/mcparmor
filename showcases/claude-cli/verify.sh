...
# Verifies MCP Armor protection is active for Claude CLI tools.
#
# Checks:
#   1. mcparmor is installed and on PATH
#   2. The broker reports active status for the claude-cli host
#   3. Secret scanning is active — a response containing a raw secret is redacted

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "=== MCP Armor Claude CLI Verification ==="
echo ""
...
# Check 2: broker status for claude-cli
echo ""
echo "=== Broker status ==="
mcparmor status --host claude-cli
